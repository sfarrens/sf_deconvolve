...
def get_coef(psf, psf_pcs, shape=None):
    ...
    if shape:
        return np.array([np.multiply(a.T, b.T).sum() for a in psf.T for b in
                         psf_pcs.T]).reshape(list(shape) + [psf_pcs.shape[2]])
    else:
        return np.array([np.multiply(a.T, b.T).sum() for a in psf.T for b in
                         psf_pcs.T]).reshape(psf.shape[2], psf_pcs.shape[2])
...
#
def psf_pca(psf, threshold, svd='sparse', return_coef=True, coef_shape=None):
